const sortImportsByComponent = (ast, options) => {
  ...
  const nonImports = ast.body.filter(
    (node) => node.type !== "ImportDeclaration"
  )
  ...
  imports.sort((a, b) => {
    if(a.specifiers.length < 2 && b.specifiers.length >= 2)
      return -1

    if(a.specifiers.length >= 2 && b.specifiers.length < 2)
      return 1

    a_specifier = a.specifiers[0].local
    b_specifier = b.specifiers[0].local
...
module.exports = {sortImportsByComponent}
